...
# 1) GeneratedContent supports Telegram forum topics.
p=Path('lib/types.ts'); s=p.read_text()
old='''export type GeneratedContent = {\n  channel: SocialChannel;\n  title: string;\n  body: string;\n  cta: string;\n  affiliateUrl: string;\n  imageUrl?: string;\n};'''
new='''export type GeneratedContent = {\n  channel: SocialChannel;\n  title: string;\n  body: string;\n  cta: string;\n  affiliateUrl: string;\n  imageUrl?: string;\n  messageThreadId?: number;\n};'''
if s.count(old)!=1: raise SystemExit('types mismatch')
p.write_text(s.replace(old,new,1))

# 2) Content templates, without exposing affiliate commission to the audience.
Path('lib/content-engine.ts').write_text(r'''import type { GeneratedContent, Offer, SocialChannel } from "./types";
...
# 3) Telegram forum topic support.
p=Path('lib/publishers/telegram.ts'); s=p.read_text()
old='''      if (content.imageUrl) {\n        const result = await telegramRequest<{ message_id: number }>(input.token, "sendPhoto", {\n          chat_id: input.chatId,\n          photo: content.imageUrl,\n          caption: caption.slice(0, 1024),\n        });'''
new='''      const thread = content.messageThreadId ? { message_thread_id: content.messageThreadId } : {};\n\n      if (content.imageUrl) {\n        const result = await telegramRequest<{ message_id: number }>(input.token, "sendPhoto", {\n          chat_id: input.chatId,\n          photo: content.imageUrl,\n          caption: caption.slice(0, 1024),\n          ...thread,\n        });'''
if s.count(old)!=1: raise SystemExit('telegram photo mismatch')
s=s.replace(old,new,1)
old='''      const result = await telegramRequest<{ message_id: number }>(input.token, "sendMessage", {\n        chat_id: input.chatId,\n        text: caption,\n        disable_web_page_preview: false,\n      });'''
new='''      const result = await telegramRequest<{ message_id: number }>(input.token, "sendMessage", {\n        chat_id: input.chatId,\n        text: caption,\n        disable_web_page_preview: false,\n        ...thread,\n      });'''
if s.count(old)!=1: raise SystemExit('telegram message mismatch')
p.write_text(s.replace(old,new,1))

# 4) Scheduler: content settings + manual run.
p=Path('src/automation.ts'); s=p.read_text()
s=s.replace('import { generateContent } from "../lib/content-engine";','import { generateContent, type ContentTemplate } from "../lib/content-engine";',1)
old='''function message(error: unknown) {\n  return error instanceof Error ? error.message : "UNKNOWN_ERROR";\n}\n'''
new='''function message(error: unknown) {\n  return error instanceof Error ? error.message : "UNKNOWN_ERROR";\n}\n\nfunction contentOptions(settings: Record<string, unknown>) {\n  const rawTemplate = typeof settings.contentTemplate === "string" ? settings.contentTemplate : "offer";\n  const template: ContentTemplate = ["offer", "natural", "storytelling", "no_price"].includes(rawTemplate)\n    ? rawTemplate as ContentTemplate\n    : "offer";\n  const threadRaw = Number(settings.telegramThreadId);\n  const messageThreadId = Number.isInteger(threadRaw) && threadRaw > 0 ? threadRaw : undefined;\n  return { template, messageThreadId };\n}\n'''
if s.count(old)!=1: raise SystemExit('automation message mismatch')
s=s.replace(old,new,1)
old='async function executeRule(db: SupabaseRest, env: Env, rule: AutomationRuleRow, slot: Date): Promise<RuleExecutionResult> {\n  const slotIso = slot.toISOString();\n  const runId = await stableUuid(`scheduler:${rule.id}:${slotIso}`);'
new='async function executeRule(db: SupabaseRest, env: Env, rule: AutomationRuleRow, slot: Date, triggerSource: "scheduler" | "manual" = "scheduler"): Promise<RuleExecutionResult> {\n  const slotIso = slot.toISOString();\n  const runId = triggerSource === "scheduler" ? await stableUuid(`scheduler:${rule.id}:${slotIso}`) : crypto.randomUUID();'
if s.count(old)!=1: raise SystemExit('execute signature mismatch')
s=s.replace(old,new,1)
s=s.replace('trigger_source: "scheduler",','trigger_source: triggerSource,',1)
s=s.replace('timezone: rule.timezone,\n      },','timezone: rule.timezone,\n        triggerSource,\n      },',1)
s=s.replace('timezone: rule.timezone,\n          unsupportedNetworks,','timezone: rule.timezone,\n          triggerSource,\n          unsupportedNetworks,',1)
s=s.replace('timezone: rule.timezone,\n        unsupportedNetworks,','timezone: rule.timezone,\n        triggerSource,\n        unsupportedNetworks,',1)
s=s.replace('content: generateContent(offer, channel),','content: generateContent(offer, channel, contentOptions(rule.settings)),',1)
s=s.replace('const content = generateContent(offer, channel);','const content = generateContent(offer, channel, contentOptions(rule.settings));',1)
old='''export async function schedulerTick(env: Env, now = new Date()) {'''
...
if s.count(old)!=1: raise SystemExit('run now insertion mismatch')
s=s.replace(old,new,1)
old='''    imageUrl: content.imageUrl ? String(content.imageUrl) : undefined,\n  };'''
new='''    imageUrl: content.imageUrl ? String(content.imageUrl) : undefined,\n    messageThreadId: Number.isInteger(Number(content.messageThreadId)) && Number(content.messageThreadId) > 0\n      ? Number(content.messageThreadId)\n      : undefined,\n  };'''
if s.count(old)!=1: raise SystemExit('contentFromQueue mismatch')
p.write_text(s.replace(old,new,1))

# 5) Replace admin with operational v2.
Path('src/admin.ts').write_text(r'''import { createShopeeProvider } from "../lib/affiliate/shopee/adapter";
...
function rulePatch(input: AdminRuleInput, creating = false, currentSettings: JsonObject = {}) {
  ...
  if (creating) {
    ...
    patch.settings={ purpose:"admin_ui", priority:100, maxAttempts:3, contentTemplate:"offer", ...(patch.settings as JsonObject || {}) };
  }
  return patch;
}
...
async function dashboard(env: Env) {
  ...
  const nextRuns=Object.fromEntries(rules.map(r=>[r.id,r.enabled?nextSlot(r.schedule_cron,r.timezone||"America/Sao_Paulo",now):null]));
...
async function telegramInfo(env: Env) {
  if(!env.TELEGRAM_BOT_TOKEN||!env.TELEGRAM_CHAT_ID) return {configured:false};
  ...
  const [me,chat]=await Promise.all([call("getMe"),call("getChat",{chat_id:chatId})]);
  return {configured:true,botUsername:me?.username||null,botName:me?.first_name||null,chatTitle:chat?.title||chat?.username||"Grupo Telegram",isForum:Boolean(chat?.is_forum),chatType:chat?.type||null};
}
