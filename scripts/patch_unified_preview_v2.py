...
# Motor único: exportar a seleção real e acrescentar estatísticas das faixas de comissão.
...
if old_head not in admin: raise SystemExit('diagnostic header not found')
admin=admin.replace(old_head,new_head,1)

# Snapshot canônico: fonte, lista, nicho/mix, comissão, agenda, destino e template em uma função.
...
# Em sucesso, exibir a faixa realmente utilizada.
...
if old_success not in admin: raise SystemExit('success preview marker not found')
admin=admin.replace(old_success,new_success,1)

auto_path.write_text(auto)
admin_path.write_text(admin)
print('patched unified preview v2')
